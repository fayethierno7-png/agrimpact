...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Connexion directe avec identifiant + mot de passe + option rester connecté
      const res = await login(cleanIdentifier, password, rememberMe);

      if (res.success) {
        // Notification administrateur en tâche de fond (asynchrone, non-bloquante)
        ...
        // 1. Si le compte est suspendu par l'administration
        if (res.profile?.statut_compte === 'suspendu') {
          setErrorMessage("Votre compte a été suspendu par l'administration. Veuillez contacter l'assistance.");
          setIsSubmitting(false);
          return;
        }

        // 2. Si le compte est en attente de validation administrative
        if (res.profile?.statut_compte === 'en_attente') {
          router.push('/en-attente');
          return;
        }

        // 3. Compte actif ou rôle admin : accès direct
        router.push(redirectUrl);
      } else {
        setErrorMessage(res.error || 'Identifiants incorrects. Veuillez vérifier vos informations.');
        setIsSubmitting(false);
      }
    } catch (err: any) {
      ...
      <div className="w-full bg-white dark:bg-stone-900 border-b border-stone-200/80 dark:border-stone-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between text-xs text-stone-600 dark:text-stone-400">
          ...
          <div className="flex items-center gap-2 font-medium text-emerald-900 dark:text-emerald-400">
            ...
            <span>Sénégal • Connexion directe sécurisée</span>
          </div>
        </div>
      </div>
